...
void DescriptorPool::next_pool()
{
    ...
    {
        auto pool_sizes = map_vec(m_pool_sizes, [&](const std::pair<VkDescriptorType, float>& sizes) {
            return VkDescriptorPoolSize{
                .type            = sizes.first,
                .descriptorCount = static_cast<uint32_t>(sizes.second * m_max_sets),
            };
        });
        ...
        VK_CHECK(vkCreateDescriptorPool(m_device, &info, nullptr, &m_current_pool));
    }
}

void DescriptorPool::reset_pools()
{

    for (auto pool : m_used_pools)
    {
        VK_CHECK(vkResetDescriptorPool(m_device, pool, 0));
...
{   
    if(m_current_pool) vkDestroyDescriptorPool(m_device, m_current_pool, nullptr);
